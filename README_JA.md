...
2. `list_running_servers()`: 現在バックグラウンドで稼働しているすべてのローカルLLMサーバー（ポート番号とモデル名）の一覧を取得します。
3. `launch_llm_server(model_name: str, port: int, memory_requirement_gb: float = 4.0)`: 背景で `mlx_lm.server` インスタンスを起動します。メモリ不足エラーを防ぐためのオプションのメモリ要件チェックが含まれています。
4. `shutdown_llm_server(port: int)`: 指定されたポートで実行中の LLM サーバーを正常に終了します。
